...
@WebServlet(urlPatterns = { "/App/RecentUploads" }, initParams = { @WebInitParam(name = "photoPerPage", value = "12") })
public class RecentUploads extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			Integer totalPhotos = photoDao.getTotalNoOfPhotosOfUser(userId);
			...
			List<Photo> photos = photoDao.getRecentPhotos(userId, offset, photoPerPage);
			request.setAttribute("photos", photos);
			request.getRequestDispatcher("/App/RecentUploads.jsp").forward(request, response);
		}
